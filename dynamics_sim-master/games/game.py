...
class SymmetricNPlayerGame(Game):
    ...
    def __init__(self, payoff_matrix, n, bias_strength = 0, bias_scale = 0, equilibrium_tolerance=0.1):
        ...
        """这行代码的作用是将一个二维列表 payoff_matrix 转置并将结果存储为元组 payoff_matrix_2。
        具体来说，zip(*payoff_matrix) 使用可变数量的参数将 payoff_matrix 列表中的元素解压缩为单独的参数，并返回一个迭代器，其中每个元素是元组。然后，map 函数将这些元组转换为元组，并将结果作为元素存储在一个元组中，这就是 payoff_matrix_2。这里使用了 tuple 和 map 函数是为了将结果转换为元组类型。
        因为这个函数返回的是一个元组嵌套的二维列表，所以 payoff_matrix_2 是一个二维元组，表示原始 payoff_matrix 的转置。    """
